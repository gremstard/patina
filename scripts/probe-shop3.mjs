...
const ps = await page.evaluate(()=>!!window.__dbg.promptShop);
await page.keyboard.press('i'); await page.waitForTimeout(120);
const invWorks = await page.evaluate(()=>document.getElementById('inv').classList.contains('open'));
await page.evaluate(()=>window.__dbg.openShop()); await page.waitForTimeout(150);
const direct = await page.evaluate(()=>window.__dbg.shopOpen);
await page.screenshot({path:join('dist','shop.png')});
await browser.close();
console.log('promptShop set:',ps,' inventory-via-I works:',invWorks,' openShop() direct:',direct);
