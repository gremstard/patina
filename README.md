...
The roadmap (§16) gates each phase on the previous one running. Phase 2 is the
pure-logic foundation, and per §5 the **determinism test came first**.
...
| **Determinism test** | §5 | `test/determinism.test.js` |

Zero runtime dependencies so far — the foundation is pure JS and runs under
Node's built-in test runner. three.js + Vite arrive with Phase 3 (rendering).

## Commands

```
npm test                 # the determinism test suite (§5) — 13 checks
npm run worldindex       # build the offline world index blob (§4), default seed
